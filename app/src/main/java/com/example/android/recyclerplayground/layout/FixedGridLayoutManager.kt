...
class FixedGridLayoutManager : RecyclerView.LayoutManager() {
    ...
    override fun onLayoutChildren(
        ...
        if (childCount == 0 && state.isPreLayout) { //Nothing to do during prelayout when empty
            return
        }
...
//Scrap measure one child
         ...
         */if (state.isPreLayout) {
            ...
            for (i in 0 until childCount) {
                ...
                if (lp.isItemRemoved) { //Track these view removals as visible
                    removedCache.put(
...
// This happens on data sets too small to scroll in a direction.
        ...
        if (!state.isPreLayout && !recycler.scrapList.isEmpty()) {
            val scrapList =
                recycler.scrapList
            val disappearingViews =
                HashSet<View>(scrapList.size)
...
// Views we still need will be added back at the proper index.
        ...
        for (i in 0 until visibleChildCount) {
            ...
            if (state.isPreLayout) {
                ...
                for (offset in 0 until removedPositions!!.size()) { //Look for off-screen removals that are less-than this
            ...
            if (nextPosition < 0 || nextPosition >= state.itemCount) { //Item space beyond the data set, don't attempt to add a view
                continue
            }
            ...
            } else { //Re-attach the cached view at its new index
                attachView(view)
    ...
    override fun scrollHorizontallyBy(
        ...
        if (viewSpan < horizontalSpace) { //We cannot scroll in either direction
            return 0
        }
        val delta: Int
        ...
        delta = if (dx > 0) { // Contents are scrolling left
            //Check right bound
            if (rightBoundReached) { //If we've reached the last column, enforce limits
                val rightOffset =
                    horizontalSpace - getDecoratedRight(bottomView) + paddingRight
                Math.max(-dx, rightOffset)
            } else { //No limits while the last column isn't visible
                -dx
            }
    ...
    override fun scrollVerticallyBy(
        ...
        if (viewSpan < verticalSpace) { //We cannot scroll in either direction
            return 0
        }
        val delta: Int
...
//Check against bottom bound
            if (bottomBoundReached) { //If we've reached the last row, enforce limits
                val bottomOffset: Int
                bottomOffset =
                    if (rowOfIndex(childCount - 1) >= maxRowCount - 1) { //We are truly at the bottom, determine how far
            ...
            } else { //No limits while the last row isn't visible
                delta = -dy
            }
...
//Check against top bound
    ...
    private fun layoutAppearingViews(
        ...
        for (extra in 1..extraCount) { //Grab the next position after the reference
            val extraPosition = referencePosition + extra
            if (extraPosition < 0 || extraPosition >= itemCount) { //Can't do anything with this
                continue
            }
            /*
